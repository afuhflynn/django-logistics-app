...
import type { LogEntry } from "@/lib/route-calculator"

interface ELDChartProps {
  logEntries: LogEntry[]
}

export default function ELDChart({ logEntries }: ELDChartProps) {
  ...
  useEffect(() => {
    ...
    const totalHours = logEntries.reduce((sum, entry) => sum + entry.duration, 0)
